  ...
  const navItems = (
    <>
      ...
      {/*<ul className="relative">
        {/* <select
          className="p-2 shadow bg-base-100 rounded-box w-47 dark:bg-slate-900 dark:text-white"
          value={selectedCategory}
          onChange={handleCategoryChange}
        > 
          {categoryData.map((category) => (
            <li key={category} value={category.toLowerCase()}>
  ...
  return (
    <div
      className={`max-w-screen-2xl container mx-auto md:px-20 px-4 dark:bg-slate-900 dark:text-white fixed top-0 left-0 right-0 z-50 ${
        sticky
          ? "sticky-navbar bg-base-300 shadow-md dark:bg-slate-400 dark:text-white transition-all duration-300 ease-in-out"
          : ""
      }`}
    >
      <div className="navbar">
        ...
        <div className="navbar-end space-x-3">
          ...
          </div>
          {
              authUser ? (<Logout />) :
                (<div>
                  <a className="bg-black text-white px-3 py-2 rounded-md hover:bg-slate-800 duration-300 cursor-pointer dark:bg-slate-100 dark:text-black"
